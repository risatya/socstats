...
var Cufon = (function() {
	...
	var n = m.CSS = {
		...
		color : j(function(C) {
					var B = {};
					B.color = C.replace(/^rgba\((.*?),\s*([\d.]+)\)/, function(
									E, D, F) {
...
Cufon.registerEngine("canvas", (function() {
	...
	return function(V, w, P, t, C, W) {
		...
		var E = V.spacing(Z, ~~m.convertFrom(parseFloat(P.get("letterSpacing"))
						|| 0), ~~m.convertFrom(parseFloat(P.get("wordSpacing"))
						|| 0));
		if (!E.length) {
			return null
		}
...
Cufon.registerEngine("vml", (function() {
	...
	return function(ac, G, Y, C, K, ad, W) {
		...
		var p = Y.computedFontSize
				|| (Y.computedFontSize = new Cufon.CSS.Size(c(ad, Y
										.get("fontSize"))
								+ "px", ac.baseSize));
		var y, q;
		if (n) {
		...
		var L = ac.spacing(ag, f(ad, Y, p, "letterSpacing"), f(ad, Y, p,
						"wordSpacing"));
		if (!L.length) {
			return null
		}
		...
		if (h) {
			...
			if (Q === undefined) {
				...
				Y.computedYAdjust = Q = 0.5
						* (a(ad, E) - parseFloat(ai.height))
			}
			if (Q) {
