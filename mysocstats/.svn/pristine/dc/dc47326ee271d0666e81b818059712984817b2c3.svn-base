...
<div class="row-fluid">
       ...
       <div class="accordion-body in collapse" id="collapse<?php echo "0" ?>">
            <table class="table table-bordered">
                    ...
                    ?>
                    <tbody>
                        <?php for ($i = 0; $i < sizeof($summarycomp); $i++) { ?>
                            <tr>
                                <?php for ($j = 0; $j < 5; $j++) { ?>
                                    <td class="span2" style="font-size: 9pt"><center><a href="<?php echo (!empty($summarycomp[$j][$i]['linkssss'])); ?>" data-toggle="tooltip" title="<?php echo (!empty($summarycomp[$j][$i]['kepanjangan'])); ?>"><?php echo (!empty($summarycomp[$j][$i]['team'])) ? $summarycomp[$j][$i]['competition'] . ' </a> - ' . '<a href="' . $summarycomp[$j][$i]['link'] . '" data-toggle="tooltip" title="' . $summarycomp[$j][$i]['team_a'] . " vs " . $summarycomp[$j][$i]['team_b'] . '">' . $summarycomp[$j][$i]['team'] . '</a>' . '<a href="' . base_url('index.php/administrator/rekap/' . $summarycomp[$j][$i]['id_negara'] . '?order=team') . '">' . ' (' . $summarycomp[$j][$i]['sum'] . ')' . '</a>' : ''; ?></center></td>    
                                    <?php// print_r($s)?>
                        <?php } ?>
                        </tr>
                    <?php } ?>
                    ...
                    <table class="table table-bordered">
                       <thead>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <?php for ($j=0; $j < 5; $j++) { ?>
                                	<td class="span2" style="font-size: 9pt"><center><a href="<?php echo (!empty($s['row'][$j][$i]['linkssss']));?>" data-toggle="tooltip" title="<?php echo (!empty($s['row'][$j][$i]['kepanjangan']));?>"><?php echo (!empty($s['row'][$j][$i]['team'])) ? $s['row'][$j][$i]['competition'].' </a> - ' . '<a href="' . $s['row'][$j][$i]['link'] . '" data-toggle="tooltip" title="' . $s['row'][$j][$i]['team_a'] . " vs " . $s['row'][$j][$i]['team_b'] . '">' . $s['row'][$j][$i]['team']. '</a>' . '<a href="' . base_url('index.php/administrator/rekapox/' . $s['row'][$j][$i]['id_negara'] . '?order=team') . '">' . ' ('.$s['row'][$j][$i]['sum'].')' . '</a>' : '' ; ?></center></td>  
                                <?php } ?>
