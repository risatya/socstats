...
//    	$this->db->select('*');
    ...
    function getsummarycomp($page = 0, $tipe, $f = 2) {
        ...
        $date = date('Y-m-d');
        $date = date('Y-m-d', strtotime('+' . $kurang . ' day', strtotime($date)));
        $sum[0] = $this->getsummarybydatecomp(date('Y-m-d', strtotime('-2 day', strtotime($date))), $tipe, $f);
        $sum[1] = $this->getsummarybydatecomp(date('Y-m-d', strtotime('-1 day', strtotime($date))), $tipe, $f);
        $sum[2] = $this->getsummarybydatecomp(date('Y-m-d', strtotime('+0 day', strtotime($date))), $tipe, $f);
        $sum[3] = $this->getsummarybydatecomp(date('Y-m-d', strtotime('+1 day', strtotime($date))), $tipe, $f);
        $sum[4] = $this->getsummarybydatecomp(date('Y-m-d', strtotime('+2 day', strtotime($date))), $tipe, $f);
        //$sum[5] = $this->getsummarybydate(date('Y-m-d', strtotime('+2 day', strtotime($date))), $id_negara, $tipe,$f);
        //$sum[6] = $this->getsummarybydate(date('Y-m-d', strtotime('+3 day', strtotime($date))), $id_negara, $tipe,$f);
